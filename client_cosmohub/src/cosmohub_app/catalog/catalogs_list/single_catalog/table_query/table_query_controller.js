...
angular.module('catalogs.controllers').controller('custom_catalog_ctrl', [ '$scope','user_auth','$state','run_query_resources', function ($scope, user_auth,$state,run_query_resources) {
  $scope.aceModel = '';
  $scope.mode = 'SQL';
  $scope.readOnly = true;
...
$scope.check_query = function() {run_query_resources.check($scope.aceModel, user_auth.api_key, user_auth.name).then(
   			function(data){ 
  ...
  $scope.realtime_query = function() {run_query_resources.run($scope.aceModel, user_auth.api_key, user_auth.name).then(
